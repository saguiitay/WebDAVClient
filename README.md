...
+ **2.7.0** _(upcoming)_  Security & hardening:
  ...
  - **Bug fix — `Overwrite` header on COPY / MOVE** (RFC 4918 §9.8.3 / §9.9.3): the client now always sends `Overwrite: T` (matching the RFC default) so server behaviour is deterministic. `MoveFile` / `MoveFolder` / `CopyFile` / `CopyFolder` gained an optional `overwrite` parameter — pass `false` to send `Overwrite: F` and protect an existing destination (server then returns `412 Precondition Failed` instead of silently clobbering). `204 No Content` is now accepted as success for COPY / MOVE (RFC 4918 §9.8.5 / §9.9.4 — returned when the destination already existed).
  - **Bug fix — `Depth: infinity` on DELETE** (RFC 4918 §9.6.1): DELETE now sends the required `Depth: infinity` header. Previously it was omitted, causing strict servers to reject collection deletes (the spec mandates the header on collections and forbids any other value).
  - **Feature — `If` lock-token submission on PUT / DELETE / MOVE / COPY** (RFC 4918 §10.4): `Upload` / `UploadPartial` / `DeleteFile` / `DeleteFolder` gained an optional `lockToken` parameter; `MoveFile` / `MoveFolder` gained `sourceLockToken` and `destinationLockToken`; `CopyFile` / `CopyFolder` gained `destinationLockToken` (COPY does not modify the source per §7.5.1, so no source token is required). Tokens are accepted in either bare (`opaquelocktoken:abc`) or `<opaquelocktoken:abc>` form. The emitted `If` header uses the no-tag-list form when only the request URI is locked and the tagged-list form (with the destination's absolute URI) when the destination is locked. Without these headers, locked-resource servers reject modifications with `423 Locked`, which is now also surfaced as `WebDAVException`.
  - **Internal refactor** — extracted the static helpers in `Client.cs` into focused helper classes under `WebDAVClient.Helpers`: `LockTokenHeaderHelper` (token normalisation + `If` / `Lock-Token` header logic), `PropPatchRequestBuilder` (property-name / namespace validation + body emission) and `XmlEscape`. No public-API change; the helpers are now directly unit-testable and the responsibilities are clearer at each call site.
  - **PROPFIND request-body variants** (RFC 4918 §9.1): in addition to the historical `<allprop/>` body, callers can now request a targeted set of properties or discover what properties a resource exposes. New overloads on `List` / `GetFolder` / `GetFile` accept an `IEnumerable<PropertyName>` (sends `<prop>`); new sibling methods `ListPropertyNames` / `GetFolderPropertyNames` / `GetFilePropertyNames` send `<propname/>`. Returned `Item` exposes three nullable collections — `FoundProperties` (status 200), `NotFoundProperties` (404 / 401 / 403 / 424) and `AvailablePropertyNames` (propname). Standard DAV: live properties still surface on `Item` typed fields (e.g. `Etag`, `IsCollection`) when queried via `<prop>`. The default `List()` / `GetFolder()` / `GetFile()` behaviour is unchanged: `<allprop/>` is still sent and the new property-bag collections stay `null`.
  - **HTTP `OPTIONS` support** (RFC 4918 §9.1, RFC 9110 §9.3.7): new `GetServerOptions(path, ct)` method on `IClient` returns a strongly-typed `ServerOptions` exposing the parsed `DAV` compliance classes (`IsClass1` / `IsClass2` / `IsClass3`, plus `HasComplianceToken` for non-numeric extensions like `access-control` or `calendar-access`) and the `Allow` methods (`SupportsMethod`). Use it to verify a resource is WebDAV-enabled (`IsWebDavServer`) before issuing `PROPFIND` / `LOCK` / etc., or to discover the server's supported method set.
  - **Bearer token / OAuth 2.0 authentication**: new `Client` constructor overloads accept either a static bearer token (`new Client("eyJ...", uploadTimeout, proxy)`) or an async token provider (`new Client(ct => tokenSource.GetTokenAsync(ct), uploadTimeout, proxy)`) for refresh / rotation flows — Azure AD / MSAL, Nextcloud app-passwords, Box, SharePoint Online, custom OAuth 2.0 token stores. Implemented via a new public `WebDAVClient.Authentication.BearerTokenAuthenticationHandler` `DelegatingHandler` that injects `Authorization: Bearer <token>` on every outbound request (including the upload pipeline); a `null` / empty token from the provider omits the header rather than throwing, so transient "no token yet" states surface as a server `401` instead of a client exception.
